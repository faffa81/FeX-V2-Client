...
{
    // Also allow prediction for local player
    ...
    if(GameClient()->Predict() && (GameClient()->AntiPingPlayers() && !GameClient()->IsOtherTeam(ClientId)) && pChar)
        return true;
    return false;
}
...
void CTrails::OnRender()
{
    ...
    // Allow trails in both online and demo playback
	...
	for(int ClientId = 0; ClientId < MAX_CLIENTS; ClientId++)
	{
        ...
        // Allow trails for both local and other players
        ...
        // Always use prediction for local player
        if(PredictPlayer || Local)
        {
            StartTick = PredTick;
